...
const TransactionTable = ({
  ...
  useEffect(() => {
    ...
    setLockedSoFar(
      Math.round(
        dummyTransactions.reduce((sum, item) => sum + item.amount, 0) * 1000
      ) / 1000
    );
    setMintedSoFar(
      Math.round(
        dummyTransactions.reduce((sum, item) => sum + item.nAmount, 0) * 1000
      ) / 1000
    );
  }, []);
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {(() => {
              ...
              return dummyTransactions.map((pair, index) => (
                <tr
                  key={`pair-${index}`}
                  className="border-b border-white/10 hover:bg-white/5 transition-colors h-full w-full"
                >
                  <td className="pt-4 pb-1 px-4 w-1/2">
                    <div className="text-xs text-white/50">{pair.date}</div>
                    <div className="flex flex-row justify-between items-center">
                      <div className="text-base">
                        {formatDisplayAddress(ethAddress)}
                      </div>
                      <div className="text-base">{`${pair.amount} ETH`}</div>
                    </div>
                  </td>
                  <td className="pt-4 pb-1 px-4 w-1/2">
                    <div className="text-xs text-white/50">{pair.date}</div>
                    <div className="flex flex-row justify-between items-center">
                      <div className="text-base">
                        {formatDisplayAddress(minaAddress ?? null)}
                      </div>
                      <div className="text-base">{`${pair.nAmount} nETH`}</div>
                    </div>
                  </td>
                </tr>
              ));
